...
class Field:
    ...
    def create_island(self, x, y):
        ...
        for i in range(y-2 if y > 1 else 0, y+3 if x < self.y-2 else self.y):
            for j in range(x-2 if x > 1 else 0,
                           x+3 if x < self.x-2 else self.x):
                ...
                if self.cells[i][j].content_type == "mine":
                    ...
                    self.cells[new_i][new_j] = \
                        cell.Cell(new_j, new_i, 0, "mine")
    ...
    def fill_field(self):
        ...
        for i in range(self.y):
            for j in range(self.x):
                ...
                if i > 0 and j > 0:
                    mines_around += \
                        (self.cells[i-1][j-1].content_type == "mine")
                if i > 0 and j < self.x - 1:
                    mines_around += \
                        (self.cells[i-1][j+1].content_type == "mine")
                if i < self.y - 1:
                    mines_around += \
                        (self.cells[i+1][j].content_type == "mine")
                ...
                if j > 0:
                    mines_around += \
                        (self.cells[i][j-1].content_type == "mine")
                if j < self.x - 1:
                    mines_around += \
                        (self.cells[i][j+1].content_type == "mine")
    ...
    def open(self, x, y):
        ...
        """
        if self.cells[y][x].content_type == "digit" and \
                self.cells[y][x].is_open():
            if self.check_all_mines_around_are_open(x, y):
                self.open_around_filled_cell(x, y)
        self.cells[y][x].open()
        if self.cells[y][x].content_type == "digit" and \
                self.cells[y][x].content.number == 0:
    ...
    def open_around_empty_cell(self, x, y):
        ...
        if y < self.y - 1 and x > 0 and not self.cells[y+1][x-1].is_open():
            self.open(x-1, y+1)
        if y < self.y - 1 and x < self.x - 1 and \
                not self.cells[y+1][x+1].is_open():
            self.open(x+1, y+1)
    ...
    def open_around_filled_cell(self, x, y):
        ...
        """
        if y > 0 and not self.cells[y-1][x].is_open() and \
                not self.cells[y-1][x].is_flagged():
            self.open(x, y-1)
        if y > 0 and x > 0 and \
                not self.cells[y-1][x-1].is_open() and \
                not self.cells[y-1][x-1].is_flagged():
            self.open(x-1, y-1)
        if y > 0 and x < self.x - 1 and \
                not self.cells[y-1][x+1].is_open() and \
                not self.cells[y-1][x+1].is_flagged():
            self.open(x+1, y-1)
        if y < self.y - 1 and not self.cells[y+1][x].is_open() and \
                not self.cells[y+1][x].is_flagged():
            self.open(x, y+1)
        if y < self.y - 1 and x > 0 and \
                not self.cells[y+1][x-1].is_open() and \
                not self.cells[y+1][x-1].is_flagged():
            self.open(x-1, y+1)
        if y < self.y - 1 and x < self.x - 1 and \
                ...
                not self.cells[y+1][x+1].is_flagged():
            self.open(x+1, y+1)
        if x > 0 and not self.cells[y][x-1].is_open() and \
                not self.cells[y][x-1].is_flagged():
            self.open(x-1, y)
        if x < self.x - 1 and not self.cells[y][x+1].is_open() and \
                not self.cells[y][x+1].is_flagged():
            self.open(x+1, y)
